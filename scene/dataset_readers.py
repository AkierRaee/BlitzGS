...
def readColmapSceneInfo_martix(path, images, eval, llffhold=97):
    ...
    if len(cam_infos) == 0:
        raise RuntimeError(
            f"No COLMAP images were found in '{os.path.join(path, reading_dir)}'. "
            "Please ensure --images points to an existing folder that contains files referenced by sparse/0/images.bin(.txt)."
        )
