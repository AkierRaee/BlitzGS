...
def get_local_running_time_by_modes(stats_collector):
    ...
    for mode in args.image_distribution_config.local_running_time_mode:
        local_running_time += stats_collector.get(mode, 0.0)
...
class DivisionStrategy:
    ...
    def get_local2j_ids(self, means2D, radii, raster_settings, cuda_args):
        ...
        local2j_ids_bool = diff_gaussian_rasterization_f._C.get_local2j_ids_bool(*args)
...
def finish_strategy_final(
    ...
    for idx, strategy in enumerate(batched_strategies):
        ...
        stats = batched_statistic_collector[idx]
        required_keys = ("forward_render_time", "backward_render_time", "forward_loss_time")
        if any(k not in stats for k in required_keys):
            missing_render_timing = True
        batched_running_time.append(
            stats.get("forward_render_time", 0.0)
            + stats.get("backward_render_time", 0.0)
            + stats.get("forward_loss_time", 0.0) * 2
        )
