...
DATA="${DATA:-/path/to/building-pixsfm}"
OUT="${OUT:-/path/to/output/building}"
NPROC="${NPROC:-4}"
...
torchrun --standalone --nnodes=1 --nproc-per-node="$NPROC" train.py \
    ...
    --spawn_gate models/spawn_gate_LOSO_building.pkl --spawn_gate_k 0.60 --spawn_gate_start 24000 \
    --sampling_factor 0.7
